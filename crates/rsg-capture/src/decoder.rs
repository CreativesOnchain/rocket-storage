...
pub async fn decode_mutator_call<P: Provider>(
    ...
    let key_bytes: [u8; 32] = args.get(..32).context("no key bytes")?.try_into().unwrap();
    ...
    let (old, new) = match op {
        ...
        StorageOp::SetString | StorageOp::DeleteString => {
            ...
            let new_val =
                if op.is_delete() { String::new() } else { parse_string_from_args(args, 32) };
            (old_val, new_val)
        }
        StorageOp::SetBytes | StorageOp::DeleteBytes => {
            ...
            let new_val =
                if op.is_delete() { "0x".into() } else { parse_bytes_from_args(args, 32) };
            (old_val, new_val)
        }
